// =============================================================================
// VoyYa Conductor — useSessionStore (Zustand)
// -----------------------------------------------------------------------------
// Mismo patrón que apps/passenger/src/state/useSessionStore.ts (DRY de PATRÓN,
// no de código — ver nota de lib/secure-storage.ts). Único store de SESIÓN:
// tokens + usuario + `status`, persistido en expo-secure-store y leído de
// forma SÍNCRONA (`getState()`) desde fuera de React (http-client.ts, vía los
// handlers inyectados abajo).
//
// Este módulo, al importarse, CONFIGURA los handlers de auth de http-client.ts
// (inversión de dependencia: http-client no conoce Zustand ni este store).
// app/_layout.tsx importa este store para llamar a `hydrate()` al arrancar.
// =============================================================================
...
interface SessionState {
  ...
  /** Login exitoso (cédula+PIN): guarda tokens + usuario. */
